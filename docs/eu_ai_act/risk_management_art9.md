...
| R3 | **Language degradation** on non-Italian content | High | Medium | Italian-optimised models documented as a limitation; input-language flag (`language.detected` in `/evaluate` and `cats.lite` responses, warning in explanations) — validated 205/205 on the July snapshot registry | Flag is response-time only (not persisted, `/explain` unaware); multilingual NLP support remains roadmap work |
