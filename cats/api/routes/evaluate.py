...
from cats.api.schemas import (
    ContestRequest, ContestResponse,
    EvaluateRequest, EvaluateResponse,
    ExplainResponse, StatsResponse,
)
...
async def evaluate(req: EvaluateRequest, request: Request, db: AsyncSession = Depends(get_db)):
    ...
    return EvaluateResponse(
        ...
        signals=[{"name": s.name, "value": s.value,
                  "confidence": s.confidence, "metadata": s.metadata} for s in signals],
    )
...
async def explain(trace_id: str, db: AsyncSession = Depends(get_db)):
    ...
    sigs = [SignalResult(name=n, value=d["value"], confidence=d["confidence"])
            for n, d in ts.signals.items()]
    return ExplainResponse(trace_id=trace_id,
...
async def stats(db: AsyncSession = Depends(get_db)):
    ...
    bands_raw = await db.execute(
        select(TrustScore.band, func.count(TrustScore.id)).group_by(TrustScore.band)
    )
